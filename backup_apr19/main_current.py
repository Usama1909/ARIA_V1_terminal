...
def get_conversation_history(user_id, limit=20):
    """Read conversation history from DB — persistent across deploys."""
    try:
        import psycopg2
        conn = psycopg2.connect(host='localhost',port=5432,dbname='aria_db',
...
def save_conversation_turn(user_id, role, content):
    """Save turn to DB — survives Railway deploys."""
    try:
        import psycopg2
        conn = psycopg2.connect(host='localhost',port=5432,dbname='aria_db',
                                user='postgres',password='aria_secure_2026')
        ...
        # Keep only last 20 turns per user
...
def get_system_context():
    """Get current system mode and world state for chat context."""
    try:
        import psycopg2
        conn = psycopg2.connect(host='localhost',port=5432,dbname='aria_db',
                                user='postgres',password='aria_secure_2026')
        ...
        cur.execute("SELECT narrative, risk_appetite, liquidity_state, dominant_driver FROM world_state ORDER BY updated_at DESC LIMIT 1")
        world_row = cur.fetchone()
        cur.close(); conn.close()
        sys_mode = json.loads(sys_row[0]) if sys_row else {'mode':'NORMAL','score':100}
        world = {'narrative': world_row[0], 'risk_appetite': world_row[1],
                 'liquidity': world_row[2], 'driver': world_row[3]} if world_row else {}
        return sys_mode, world
    except:
        return {'mode':'NORMAL','score':100}, {}
...
def ask_aria(user_message, symbol=None, user_level='intermediate', messages=None):
    ...
    sys_mode, world = get_system_context()
    system_status = f"SYSTEM MODE: {sys_mode.get('mode','NORMAL')} (health:{sys_mode.get('score',100)}/100)"
    world_status  = f"WORLD STATE: {world.get('narrative','UNKNOWN')} | Risk:{world.get('risk_appetite','?')} | Liquidity:{world.get('liquidity','?')}" if world else ""
...
6. If system mode is SAFE or DEGRADED, explain what anomaly triggered it
7. Reference world state narrative when relevant to the user question
{system_status}
{world_status}
LIVE NEWS:
{news_context}
...
Fear/Greed: {fg_value}"""
    # Use proper messages array for multi-turn conversation
...
def chat(request: ChatRequest):
    ...
    try:
        ...
        # Load history from DB — persistent across deploys
        history = get_conversation_history(user_id)
        # Save user message to DB
        save_conversation_turn(user_id, "user", request.message)
        # Build proper messages array for Claude API
        ...
        # Also update RAM cache for backward compat
