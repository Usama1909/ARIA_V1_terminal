# main.py - ARIA Terminal v2 - Complete Fixed Version
# Fixes: Railway PostgreSQL, get_db conflict, portfolio persistence, positions webhook
...
def init_railway_db():
    if not RAILWAY_DB_URL:
        print("Railway DB: No DATABASE_URL set")
        return
    try:
        conn = get_railway_db()
        cur = conn.cursor()
        cur.execute("""
            CREATE TABLE IF NOT EXISTS paper_portfolios_v2 (
                user_id VARCHAR(100) PRIMARY KEY,
                balance FLOAT DEFAULT 10000,
                starting_balance FLOAT DEFAULT 10000,
                open_trades JSONB DEFAULT '[]',
                closed_trades JSONB DEFAULT '[]',
...
def save_portfolio_to_db(user_id, portfolio):
    if not RAILWAY_DB_URL:
        return
    try:
        conn = get_railway_db()
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO paper_portfolios_v2 (user_id, balance, starting_balance, open_trades, closed_trades)
            VALUES (%s, %s, %s, %s, %s)
        ...
        """, (
            ...
            json.dumps(portfolio['open_trades']),
            json.dumps(portfolio['closed_trades'])
        ))
        conn.commit()
        cur.close()
        conn.close()
...
def load_portfolio_from_db(user_id):
    ...
    try:
        conn = get_railway_db()
        cur = conn.cursor()
        cur.execute("SELECT balance, starting_balance, open_trades, closed_trades FROM paper_portfolios_v2 WHERE user_id = %s", (user_id,))
        row = cur.fetchone()
        cur.close()
        conn.close()
        if row:
            return {
                ...
                'open_trades': row[2] if isinstance(row[2], list) else json.loads(row[2] or '[]'),
                'closed_trades': row[3] if isinstance(row[3], list) else json.loads(row[3] or '[]'),
            }
    except Exception as e:
...
def build_default_sentiment():
    ...
    for symbol in ['BTC', 'ETH', 'AAPL', 'NVDA', 'TSLA', 'GLD']:
        try:
            yf_map = {'BTC': 'BTC-USD', 'ETH': 'ETH-USD', 'AAPL': 'AAPL', 'NVDA': 'NVDA', 'TSLA': 'TSLA', 'GLD': 'GLD'}
            ...
            default[symbol] = {'sentiment_label': label, 'composite_score': score, 'news_count': 0,
                               'fear_greed_value': fg_value, 'fear_greed_label': 'Extreme Fear' if fg_value < 25 else 'Fear', 'top_headlines': []}
        except:
            default[symbol] = {'sentiment_label': 'NEUTRAL', 'composite_score': 0, 'news_count': 0,
                               'fear_greed_value': 23, 'fear_greed_label': 'Extreme Fear', 'top_headlines': []}
    return default
...
def load_signal_history():
    if os.path.exists(SIGNAL_HISTORY_FILE):
        try:
            with open(SIGNAL_HISTORY_FILE, 'r') as f: return json.load(f)
        except: return []
    return []
...
def save_signal_to_history(symbol, signal, confidence, price, sentiment, rsi):
    try:
        ...
        if session_key in _logged_this_session: return
        ...
        for existing in history[-20:]:
            if existing['symbol'] == symbol:
                ...
                if (now - existing_time).total_seconds() / 60 < 60: return
        history.append({'timestamp': now.isoformat(), 'symbol': symbol, 'signal': signal,
                       'confidence': round(confidence * 100, 1), 'price': round(price, 4),
                       'sentiment': sentiment, 'rsi': round(rsi, 1), 'outcome': 'PENDING'})
        with open(SIGNAL_HISTORY_FILE, 'w') as f: json.dump(history, f, indent=2)
    except Exception as e:
...
def save_paper_portfolios(portfolios):
    try:
        with open(PAPER_TRADES_FILE, 'w') as f: json.dump(portfolios, f, indent=2)
    except: pass

def get_or_create_portfolio(user_id):
    db_portfolio = load_portfolio_from_db(user_id)
    if db_portfolio:
        return db_portfolio
    portfolios = load_paper_portfolios()
    if user_id not in portfolios:
        portfolios[user_id] = {
            'user_id': user_id, 'balance': STARTING_BALANCE, 'starting_balance': STARTING_BALANCE,
            'open_trades': [], 'closed_trades': [], 'created_at': datetime.now().isoformat()
        }
        save_paper_portfolios(portfolios)
    return portfolios[user_id]

def persist_portfolio(user_id, portfolio):
    save_portfolio_to_db(user_id, portfolio)
    portfolios = load_paper_portfolios()
...
def get_live_features(symbol):
    ...
    feature_names = ['rsi','macd','macd_hist','volatility','bb_position','ma_distance',
    ...
    features = pd.DataFrame({
        ...
        'bb_position': bb_position, 'ma_distance': ma_distance, 'price_change_5': price_change_5,
        'price_change_10': price_change_10, 'price_change_24': price_change_24, 'rsi_momentum': rsi_momentum,
        ...
        'range_position': pd.Series(range_position, index=prices.index), 'candle_range': candle_range,
        ...
        'adx_proxy': adx_proxy, 'z_score': z_score, 'momentum_5': momentum_5, 'momentum_10': momentum_10,
        'atr_pct': atr_pct, 'vpin_norm': vpin_norm, 'vpin_signal': pd.Series(vpin_signal, index=prices.index)
    })
    ...
    return latest[feature_names].values.reshape(1, -1), {
        ...
        'ma_distance': float(ma_distance.dropna().iloc[-1]), 'volatility': float(volatility.dropna().iloc[-1]),
...
def get_model_signal(symbol, features_array):
    if symbol not in production_models: return None
    ...
    try:
        ...
        return {'signal': 'TAKE_PROFIT' if tp_prob > sl_prob else 'STOP_LOSS',
                'confidence': max(tp_prob, sl_prob), 'tp_probability': tp_prob, 'sl_probability': sl_prob}
    except: return None
...
def get_live_price_only(symbol):
    ...
    try:
        ...
        if not market_data: return _price_cache.get(symbol, None)
        ...
        price = market_data['price']; price_24h = market_data['price_24h_ago']
        change_24h = ((price - price_24h) / price_24h) * 100
        result = {'symbol': symbol, 'price': round(price, 4), 'change_24h': round(change_24h, 2),
                  'rsi': round(market_data['rsi'], 2), 'signal': model_signal,
                  'sentiment': sentiment.get('sentiment_label', 'NEUTRAL'),
                  'fear_greed': sentiment.get('fear_greed_value', 50), 'timestamp': datetime.now().isoformat()}
        _price_cache[symbol] = result; _price_cache_time[symbol] = now
        return result
    except: return _price_cache.get(symbol, None)

def build_aria_context(symbol):
    features_array, market_data = get_live_features(symbol)
    if features_array is None: return None
    model_signal = get_model_signal(symbol, features_array)
    sentiment = live_sentiment.get(symbol, {'sentiment_label': 'NEUTRAL', 'composite_score': 0,
                                           'news_count': 0, 'fear_greed_value': 50, 'fear_greed_label': 'Neutral', 'top_headlines': []})
    ...
    if model_signal and model_signal['confidence'] >= 0.55:
        save_signal_to_history(symbol, model_signal['signal'], model_signal['confidence'], price,
                              sentiment.get('sentiment_label', 'NEUTRAL'), market_data['rsi'])
    return {'symbol': symbol, 'price': price, 'change_24h': ((price - price_24h) / price_24h) * 100,
            'atr': atr, 'atr_pct': (atr/price)*100, 'rsi': market_data['rsi'], 'macd': market_data['macd'],
            'ma_distance': market_data['ma_distance'], 'volatility': market_data['volatility'],
            'vpin_norm': market_data['vpin_norm'], 'tp_level': price + (atr * barriers['tp_mult']),
            'sl_level': price - (atr * barriers['sl_mult']), 'model_signal': model_signal,
...
def run_backtest(symbol):
    ...
    try:
        ...
        ticker = yf.Ticker(yf_map[symbol])
        df = ticker.history(period='60d', interval='1h')
        if len(df) < 200: raise ValueError("Not enough data")
        ...
        wins = []; losses = []; win_pcts = []; loss_pcts = []; equity = [10000.0]; trades = []
        for i in range(50, len(prices) - horizon - 1, 24):
            try:
                slice_df = df.iloc[max(0, i-60):i+1].copy()
                slice_prices = slice_df['Close']; slice_volume = slice_df['Volume']
                slice_high = slice_df['High']; slice_low = slice_df['Low']
                if len(slice_prices) < 50: continue
                tr1 = slice_high - slice_low; tr2 = abs(slice_high - slice_prices.shift(1)); tr3 = abs(slice_low - slice_prices.shift(1))
                ...
                entry_price = prices[i]; tp = entry_price + current_atr * barriers['tp_mult']; sl = entry_price - current_atr * barriers['sl_mult']
                delta = slice_prices.diff()
                gain = delta.where(delta > 0, 0).rolling(14).mean(); loss = -delta.where(delta < 0, 0).rolling(14).mean()
                rsi = 100 - (100 / (1 + gain / loss))
                ema_fast = slice_prices.ewm(span=12).mean(); ema_slow = slice_prices.ewm(span=26).mean()
                macd = ema_fast - ema_slow; macd_hist = macd - macd.ewm(span=9).mean()
                ma_20 = slice_prices.rolling(20).mean(); ma_50 = slice_prices.rolling(50).mean()
                std_20 = slice_prices.rolling(20).std(); bb_upper = ma_20 + std_20 * 2; bb_lower = ma_20 - std_20 * 2
                volatility = slice_prices.pct_change().rolling(10).std()
                bb_position = (slice_prices - bb_lower) / (bb_upper - bb_lower); ma_distance = (ma_20 - ma_50) / ma_50
                price_change_5 = slice_prices.pct_change(5); price_change_10 = slice_prices.pct_change(10)
                price_change_24 = slice_prices.pct_change(min(24, len(slice_prices)-1))
                rsi_momentum = rsi.diff(); volume_ratio = slice_volume / slice_volume.rolling(20).mean()
                volume_trend = slice_volume.rolling(5).mean() / slice_volume.rolling(20).mean()
                prices_4h = slice_prices.rolling(4).mean(); delta_4h = prices_4h.diff()
                gain_4h = delta_4h.where(delta_4h > 0, 0).rolling(14).mean(); loss_4h = -delta_4h.where(delta_4h < 0, 0).rolling(14).mean()
                rsi_4h = 100 - (100 / (1 + gain_4h / loss_4h))
                high_24 = slice_prices.rolling(min(24, len(slice_prices))).max(); low_24 = slice_prices.rolling(min(24, len(slice_prices))).min()
                dist_from_high = (slice_prices - high_24) / high_24; dist_from_low = (slice_prices - low_24) / low_24
                range_24 = high_24 - low_24; range_position = np.where(range_24 > 0, (slice_prices - low_24) / range_24, 0.5)
                ...
                momentum_5 = slice_prices / slice_prices.shift(5) - 1; momentum_10 = slice_prices / slice_prices.shift(10) - 1
                ...
                buy_volume = slice_volume * buy_vol_frac; ofi = abs(buy_volume - slice_volume * (1 - buy_vol_frac))
                ...
                feat_df = pd.DataFrame({
                    ...
                    'volume_ratio': volume_ratio, 'volume_trend': volume_trend, 'rsi_4h': rsi_4h,
                    'dist_from_high': dist_from_high, 'dist_from_low': dist_from_low,
                    'range_position': pd.Series(range_position, index=slice_prices.index), 'candle_range': candle_range,
                    ...
                    'adx_proxy': adx_proxy, 'z_score': z_score, 'momentum_5': momentum_5, 'momentum_10': momentum_10,
                    'atr_pct': atr_pct, 'vpin_norm': vpin_norm, 'vpin_signal': pd.Series(vpin_signal, index=slice_prices.index)
                })
                ...
                outcome = None; exit_price = future_prices[-1] if len(future_prices) > 0 else entry_price
                for fp in future_prices:
                    if direction == 'TAKE_PROFIT':
                        if fp >= tp: outcome = 'WIN'; exit_price = tp; break
                        elif fp <= sl: outcome = 'LOSS'; exit_price = sl; break
                    else:
                        if fp <= sl: outcome = 'WIN'; exit_price = sl; break
                        elif fp >= tp: outcome = 'LOSS'; exit_price = tp; break
                if outcome is None:
                    exit_price = future_prices[-1] if len(future_prices) > 0 else entry_price
                    if direction == 'TAKE_PROFIT': outcome = 'WIN' if exit_price > entry_price else 'LOSS'
                    else: outcome = 'WIN' if exit_price < entry_price else 'LOSS'
                if direction == 'TAKE_PROFIT': pnl_pct = (exit_price - entry_price) / entry_price * 100
                else: pnl_pct = (entry_price - exit_price) / entry_price * 100
                pnl_usd = (pnl_pct / 100) * 100
                if outcome == 'WIN': wins.append(pnl_usd); win_pcts.append(abs(pnl_pct))
                else: losses.append(pnl_usd); loss_pcts.append(abs(pnl_pct))
                equity.append(equity[-1] + pnl_usd)
                trades.append({'direction': direction, 'outcome': outcome, 'pnl_pct': round(pnl_pct, 2), 'confidence': round(signal['confidence'], 3)})
            except: continue
        total = len(wins) + len(losses)
        if total == 0: raise ValueError("No trades generated")
        ...
        drawdown = (equity_arr - peak) / peak * 100; max_dd = float(np.min(drawdown))
        total_wins = sum(abs(w) for w in wins); total_losses = sum(abs(l) for l in losses)
        profit_factor = round(total_wins / total_losses, 2) if total_losses > 0 else 0
        result = {'symbol': symbol, 'total_trades': total, 'win_rate': round(win_rate * 100, 1),
                  'avg_win_pct': round(avg_win_pct, 2), 'avg_loss_pct': round(avg_loss_pct, 2),
                  'sharpe_ratio': round(sharpe, 2), 'max_drawdown': round(max_dd, 2), 'profit_factor': profit_factor,
                  'total_pnl': round(sum(wins) + sum(losses), 2), 'equity_curve': [round(e, 2) for e in equity[-50:]],
                  'source': 'backtest_60d_live', 'timestamp': datetime.now().isoformat()}
        _backtest_cache[symbol] = result; _backtest_cache_time[symbol] = now
        print(f"  Backtest {symbol}: {total} trades, {win_rate*100:.1f}% win rate, Sharpe {sharpe:.2f}")
        return result
    except Exception as e:
        print(f"Backtest error {symbol}: {e}")
        defaults = {'BTC': {'win_rate': 52.0, 'avg_win_pct': 1.8, 'avg_loss_pct': 1.2, 'sharpe_ratio': 0.8},
        ...
        return {**d, 'symbol': symbol, 'total_trades': 0, 'max_drawdown': -15.0, 'profit_factor': 1.2,
                'total_pnl': 0, 'equity_curve': [], 'source': 'default', 'timestamp': datetime.now().isoformat()}
...
def get_macro_indicators():
    ...
    now = time.time()
    if _macro_cache and (now - _macro_cache_time) < 3600: return _macro_cache
    try:
        indicators = {}
        try:
            tnx = yf.Ticker('^TNX'); irx = yf.Ticker('^IRX')
            hist_10y = tnx.history(period='30d', interval='1d'); hist_2y = irx.history(period='30d', interval='1d')
            if len(hist_10y) > 0 and len(hist_2y) > 0:
                rate_10y = float(hist_10y['Close'].iloc[-1]); rate_2y = float(hist_2y['Close'].iloc[-1]) / 100
                spread = rate_10y - rate_2y
                indicators['yield_curve'] = {'value': round(spread, 3), 'rate_10y': round(rate_10y, 3),
                    'rate_2y': round(rate_2y * 100, 3), 'signal': 'INVERTED' if spread < 0 else 'NORMAL',
                    'warning': spread < 0, 'description': 'Yield curve inverted' if spread < 0 else 'Yield curve normal', 'burry_signal': spread < -0.5}
        except:
            indicators['yield_curve'] = {'value': 0, 'signal': 'UNAVAILABLE', 'warning': False, 'burry_signal': False}
        try:
            vix = yf.Ticker('^VIX'); vix_hist = vix.history(period='30d', interval='1d')
            if len(vix_hist) > 0:
                vix_current = float(vix_hist['Close'].iloc[-1]); vix_avg = float(vix_hist['Close'].mean())
                indicators['vix'] = {'current': round(vix_current, 2), 'avg_30d': round(vix_avg, 2),
                    'signal': 'EXTREME_FEAR' if vix_current > 35 else 'HIGH_FEAR' if vix_current > 25 else 'ELEVATED' if vix_current > 18 else 'NORMAL',
                    'warning': vix_current > 25, 'description': f'VIX at {vix_current:.1f}', 'burry_signal': vix_current > 30}
        except:
            indicators['vix'] = {'current': 20, 'signal': 'UNAVAILABLE', 'warning': False, 'burry_signal': False}
        try:
            hyg = yf.Ticker('HYG'); lqd = yf.Ticker('LQD')
            hyg_hist = hyg.history(period='30d', interval='1d'); lqd_hist = lqd.history(period='30d', interval='1d')
            if len(hyg_hist) > 0 and len(lqd_hist) > 0:
                ...
                indicators['credit_spreads'] = {'hyg_return_30d': round(hyg_return, 2), 'lqd_return_30d': round(lqd_return, 2),
                    'spread': round(spread, 2), 'signal': 'STRESS' if spread < -2 else 'ELEVATED' if spread < -0.5 else 'NORMAL',
                    'warning': spread < -2, 'description': 'Credit spreads', 'burry_signal': spread < -3}
        except:
            indicators['credit_spreads'] = {'spread': 0, 'signal': 'UNAVAILABLE', 'warning': False, 'burry_signal': False}
        try:
            dxy = yf.Ticker('DX-Y.NYB'); dxy_hist = dxy.history(period='30d', interval='1d')
            if len(dxy_hist) > 0:
                dxy_current = float(dxy_hist['Close'].iloc[-1]); dxy_change = float((dxy_hist['Close'].iloc[-1] / dxy_hist['Close'].iloc[0] - 1) * 100)
                indicators['dollar_index'] = {'current': round(dxy_current, 2), 'change_30d': round(dxy_change, 2),
                    'signal': 'STRONG' if dxy_change > 3 else 'WEAK' if dxy_change < -3 else 'NEUTRAL',
                    'warning': abs(dxy_change) > 5, 'description': 'Dollar index', 'burry_signal': dxy_change > 5}
        except:
            indicators['dollar_index'] = {'current': 100, 'signal': 'UNAVAILABLE', 'warning': False, 'burry_signal': False}
        try:
            gld = yf.Ticker('GLD'); btc = yf.Ticker('BTC-USD')
            gld_hist = gld.history(period='30d', interval='1d'); btc_hist = btc.history(period='30d', interval='1d')
            if len(gld_hist) > 0 and len(btc_hist) > 0:
                ...
                indicators['gold_vs_crypto'] = {'gold_30d': round(gld_chg, 2), 'btc_30d': round(btc_chg, 2),
                    'signal': 'RISK_OFF' if gld_chg > btc_chg + 5 else 'RISK_ON' if btc_chg > gld_chg + 5 else 'NEUTRAL',
                    'description': 'Gold vs crypto', 'burry_signal': gld_chg > btc_chg + 10}
        except:
        ...
        warnings_count = sum(1 for k, v in indicators.items() if v.get('warning', False))
        crisis_score = min(100, burry_signals * 25 + warnings_count * 10)
        if crisis_score >= 75: overall = 'CRISIS'
        elif crisis_score >= 50: overall = 'HIGH_RISK'
        elif crisis_score >= 25: overall = 'ELEVATED'
        else: overall = 'NORMAL'
        indicators['summary'] = {'crisis_score': crisis_score, 'overall': overall, 'burry_signals': burry_signals,
            'warnings': warnings_count, 'interpretation': f'{burry_signals} Burry-type signals active.', 'timestamp': datetime.now().isoformat()}
        _macro_cache = indicators; _macro_cache_time = now
        return indicators
    except Exception as e:
...
def fetch_news_background():
    ...
    RSS_FEEDS = [('https://feeds.bbci.co.uk/news/world/rss.xml', 'bbc'),
                 ('https://cointelegraph.com/rss', 'cointelegraph'),
                 ('https://feeds.a.dj.com/rss/RSSMarketsMain.xml', 'wsj'),
                 ('https://decrypt.co/feed', 'decrypt')]
    KEYWORDS = {'war': ['war','conflict','attack','missile','troops','military'],
                'fed': ['fed','federal reserve','interest rate','inflation','powell'],
                'crypto': ['bitcoin','ethereum','crypto','btc','eth'],
                'markets': ['stock','market','nasdaq','s&p','dow','equity']}
    while True:
        try:
            all_headlines = []; seen_titles = set()
            for url, source in RSS_FEEDS:
                try:
                    ...
                    for item in root.findall('.//item')[:8]:
                        ...
                        title_lower = title.lower(); tag = 'markets'; priority = 0
                        for kw_tag, keywords in KEYWORDS.items():
                            if any(kw in title_lower for kw in keywords):
                                tag = kw_tag; priority = 2 if kw_tag in ['war','fed','crypto'] else 1; break
            ...
            news_cache['headlines'] = all_headlines[:47]; news_cache['last_updated'] = datetime.now().strftime('%H:%M UTC')
...
# ── RISK ENGINE ───────────────────────────────────────────
SLIPPAGE_MODEL = {'BTC': 0.0005, 'ETH': 0.0007, 'AAPL': 0.0003, 'NVDA': 0.0005, 'TSLA': 0.0006, 'GLD': 0.0004}
BID_ASK_SPREAD = {'BTC': 0.0001, 'ETH': 0.0002, 'AAPL': 0.0001, 'NVDA': 0.0002, 'TSLA': 0.0003, 'GLD': 0.0002}
REGIME_KELLY_MULTIPLIER = {'CRISIS': 0.25, 'BEAR': 0.50, 'SIDEWAYS': 0.75, 'BULL': 1.00, 'EUPHORIA': 0.50}

def clean_floats(obj):
    if isinstance(obj, dict): return {k: clean_floats(v) for k, v in obj.items()}
    elif isinstance(obj, list): return [clean_floats(v) for v in obj]
...
def detect_market_regime(symbol):
    try:
        ...
        if not price_data: return 'SIDEWAYS'
        fg = price_data.get('fear_greed', 50); rsi = price_data.get('rsi', 50)
...
def get_realised_volatility(symbol):
    yf_map = {'BTC': 'BTC-USD', 'ETH': 'ETH-USD', 'AAPL': 'AAPL', 'NVDA': 'NVDA', 'TSLA': 'TSLA', 'GLD': 'GLD'}
    try:
        ticker = yf.Ticker(yf_map[symbol]); df = ticker.history(period='30d', interval='1h')
        returns = df['Close'].pct_change().dropna(); hourly_vol = float(returns.std())
        daily_vol = hourly_vol * np.sqrt(24); annual_vol = daily_vol * np.sqrt(252)
        df_t, _, _ = student_t.fit(returns.values); df_t = max(3, min(30, df_t))
        return {'daily_vol': round(float(daily_vol), 4), 'annual_vol': round(float(annual_vol * 100), 1),
                'hourly_vol': round(float(hourly_vol), 4), 'student_t_df': round(float(df_t), 1), 'is_crypto': symbol in ['BTC', 'ETH']}
    except:
        defaults = {'BTC': 0.025, 'ETH': 0.030, 'AAPL': 0.012, 'NVDA': 0.020, 'TSLA': 0.025, 'GLD': 0.008}
        return {'daily_vol': defaults.get(symbol, 0.02), 'annual_vol': defaults.get(symbol, 0.02) * np.sqrt(252) * 100,
                'hourly_vol': defaults.get(symbol, 0.02) / np.sqrt(24), 'student_t_df': 4.0, 'is_crypto': symbol in ['BTC', 'ETH']}

def get_backtested_win_rates(symbol):
    now = time.time()
    if symbol in _backtest_cache and (now - _backtest_cache_time.get(symbol, 0)) < 3600:
        bt = _backtest_cache[symbol]
        return {'win_rate': bt['win_rate'] / 100, 'avg_win_pct': bt['avg_win_pct'],
                'avg_loss_pct': bt['avg_loss_pct'], 'sample_size': bt['total_trades'], 'source': bt['source']}
    defaults = {'BTC': {'win_rate': 0.52, 'avg_win_pct': 1.8, 'avg_loss_pct': 1.2},
...
def kelly_criterion(win_rate, avg_win_pct, avg_loss_pct, regime='SIDEWAYS', max_fraction=0.20):
    if avg_loss_pct <= 0 or win_rate <= 0 or win_rate >= 1: return 0.02
    b = avg_win_pct / avg_loss_pct; p = win_rate; q = 1 - win_rate
    kelly = (b * p - q) / b; multiplier = REGIME_KELLY_MULTIPLIER.get(regime, 0.75)
...
def monte_carlo_simulation(entry_price, direction, vol_data, holding_period_hours, trade_cost_pct, num_simulations=10000):
    ...
    price_paths = entry_price * np.exp(np.cumsum(hourly_returns, axis=1)); final_prices = price_paths[:, -1]
    if direction == 'LONG': returns = (final_prices - entry_price) / entry_price * 100
    else: returns = (entry_price - final_prices) / entry_price * 100
    returns = returns - (trade_cost_pct * 100); profitable = returns > 0
    return {'probability_profit': round(float(np.mean(profitable)) * 100, 1),
            'expected_return_pct': round(float(np.mean(returns)), 2), 'median_return_pct': round(float(np.median(returns)), 2),
            'best_case_pct': round(float(np.percentile(returns, 95)), 2), 'worst_case_pct': round(float(np.percentile(returns, 5)), 2),
            'std_dev_pct': round(float(np.std(returns)), 2), 'skewness': round(float(stats.skew(returns)), 2),
            'kurtosis': round(float(stats.kurtosis(returns)), 2), 'distribution': f'Student-t (df={df_t:.1f})', 'simulations_run': num_simulations}
...
def calculate_var_cvar(position_usd, vol_data, confidence=0.95, horizon_days=1):
    ...
    try:
        ...
        cutoff = student_t.ppf(1 - confidence, df=df_t); tail = samples[samples < cutoff]
        cvar = position_usd * abs(float(np.mean(tail))) * np.sqrt(horizon_days)
        if math.isnan(cvar) or math.isinf(cvar): cvar = var * 1.3
    except: cvar = var * 1.3
    return {'var_95': round(float(var), 2), 'cvar_95': round(float(cvar), 2),
            'var_99': round(float(position_usd * daily_vol * abs(student_t.ppf(0.01, df=df_t))), 2), 'method': 'Student-t (fat tails)'}

def analyse_trade_risk(symbol, direction, amount_usd, entry_price, portfolio_balance, signal_confidence, num_simulations=1000):
    regime = detect_market_regime(symbol); regime_mult = REGIME_KELLY_MULTIPLIER.get(regime, 0.75)
    vol_data = get_realised_volatility(symbol); win_data = get_backtested_win_rates(symbol)
    ...
    ev_usd = round((ev_pct / 100) * amount_usd, 2); rr_ratio = round(avg_win_pct / avg_loss_pct, 2) if avg_loss_pct > 0 else 0
    ...
    risk_score = min(100, int((min(position_pct / max(kelly_pct, 0.1), 3) * 25) + (var_cvar['var_95'] / amount_usd * 100 * 25) +
                              ((1 - mc['probability_profit'] / 100) * 25) + ((1 - signal_confidence) * 25)))
    lines = []
    ...
    if ev_usd <= 0: lines.append(f"NEGATIVE EV: Expected value ${ev_usd} after costs.")
    ...
    else: lines.append(f"MARGINAL: {mc['probability_profit']}% profit probability.")
    return clean_floats({
        'symbol': symbol, 'direction': direction, 'amount_usd': amount_usd,
        'regime': {'current': regime, 'multiplier': regime_mult},
        'volatility': {'daily_pct': round(vol_data['daily_vol'] * 100, 2), 'annual_pct': vol_data['annual_vol']},
        'kelly': {'optimal_fraction': kelly_frac, 'optimal_pct': round(kelly_pct, 1), 'optimal_amount': round(kelly_amount, 2),
                 'your_pct': round(position_pct, 1), 'assessment': sizing},
        'win_rate_data': {'win_rate': round(win_rate * 100, 1), 'avg_win_pct': avg_win_pct, 'avg_loss_pct': avg_loss_pct,
                         'rr_ratio': rr_ratio, 'sample_size': win_data['sample_size'], 'source': win_data['source']},
        'monte_carlo': mc, 'risk_metrics': {**var_cvar, 'expected_value_usd': ev_usd, 'risk_score': risk_score},
        'recommendation': " ".join(lines), 'timestamp': datetime.now().isoformat()
    })
    ...
    for trade in portfolio['open_trades']:
        ...
        try:
            ...
            if price_data:
                ...
                if direction == 'LONG': pnl_pct = ((current_price - entry_price) / entry_price) * 100
                else: pnl_pct = ((entry_price - current_price) / entry_price) * 100
                pnl_usd = (pnl_pct / 100) * amount_usd
                trade['current_price'] = round(current_price, 4); trade['pnl_pct'] = round(pnl_pct, 2)
                trade['pnl_usd'] = round(pnl_usd, 2); total_unrealised += pnl_usd
        except:
...
def check_rate_limit(user_id, limit=50):
    now = time.time()
    if user_id not in usage_tracker: usage_tracker[user_id] = []
    usage_tracker[user_id] = [t for t in usage_tracker[user_id] if now - t < 3600]
    if len(usage_tracker[user_id]) >= limit: return False
    usage_tracker[user_id].append(now); return True

def get_live_news_context():
    try:
        headlines = news_cache.get('headlines', [])[:12]
        if not headlines: return "No live news available."
...
def get_price(symbol: str):
    ...
    if not data: raise HTTPException(status_code=503, detail="Could not fetch data")
    return data
...
def get_sentiment():
    ...
    for symbol, data in live_sentiment.items():
        summary[symbol] = {"label": data.get('sentiment_label', 'NEUTRAL'), "score": data.get('composite_score', 0),
                          "news_count": data.get('news_count', 0), "fear_greed": data.get('fear_greed_value', 50)}
    return {"sentiment": summary, "timestamp": datetime.now().isoformat()}

@app.get("/news")
def get_news():
    return {"headlines": news_cache.get('headlines', []), "count": len(news_cache.get('headlines', [])),
            "last_updated": news_cache.get('last_updated', 'loading...'), "timestamp": datetime.now().isoformat()}
...
def get_risk_analysis(symbol: str, direction: str = 'LONG', amount_usd: float = 100.0, user_id: str = 'aria-user'):
    ...
    try:
        price_data = get_live_price_only(symbol)
        if not price_data: raise HTTPException(status_code=503, detail="Could not fetch price")
        signal = price_data.get('signal', {}); confidence = signal.get('confidence', 0.5) if signal else 0.5
        portfolio = get_or_create_portfolio(user_id)
        return analyse_trade_risk(symbol, direction.upper(), amount_usd, price_data['price'], portfolio['balance'], confidence)
...
def get_backtest(symbol: str):
    ...
    if symbol not in ['BTC','ETH','AAPL','NVDA','TSLA','GLD']:
        raise HTTPException(status_code=400, detail="Invalid symbol")
    try: return clean_floats(run_backtest(symbol))
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def get_all_backtests():
    results = {}
    for symbol in ['BTC','ETH','AAPL','NVDA','TSLA','GLD']:
        try: results[symbol] = run_backtest(symbol)
        except Exception as e: results[symbol] = {'error': str(e)}
    return clean_floats(results)

@app.get("/macro")
def get_macro():
    try: return clean_floats(get_macro_indicators())
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))

@app.get("/riskportfolio/{user_id}")
def get_portfolio_risk(user_id: str):
    try:
        portfolio = get_or_create_portfolio(user_id); closed = portfolio['closed_trades']
        if len(closed) < 2: return {"message": "Need at least 2 closed trades for analytics"}
        pnl_list = [t.get('pnl_usd', 0) for t in closed]; wins = [p for p in pnl_list if p > 0]
        return clean_floats({'total_trades': len(closed), 'win_rate': round(len(wins) / len(closed) * 100, 1),
                            'total_pnl': round(sum(pnl_list), 2), 'timestamp': datetime.now().isoformat()})
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def open_paper_trade(request: TradeRequest):
    ...
    try:
        ...
        trade = {'trade_id': trade_id, 'symbol': symbol, 'direction': direction,
        ...
        persist_portfolio(request.user_id, portfolio)
        return {"success": True, "trade": trade, "balance": round(portfolio['balance'], 2),
                "message": f"Opened {direction} on {symbol} @ ${current_price:.2f}"}
    except HTTPException: raise
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def get_paper_portfolio(user_id: str):
    try:
        ...
        closed = portfolio['closed_trades']; wins = [t for t in closed if t.get('pnl_usd', 0) > 0]
        win_rate = round(len(wins) / len(closed) * 100, 1) if closed else 0
        return {"user_id": user_id, "balance": round(portfolio['balance'], 2), "starting_balance": STARTING_BALANCE,
                "total_value": round(total_value, 2), "total_return_pct": round(total_return, 2),
                "unrealised_pnl": round(unrealised_pnl, 2), "open_trades": portfolio['open_trades'],
                "open_count": len(portfolio['open_trades']), "closed_count": len(closed),
                "win_rate": win_rate, "timestamp": datetime.now().isoformat()}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def close_paper_trade(request: CloseTradeRequest):
    try:
        ...
        persist_portfolio(request.user_id, portfolio)
        return {"success": True, "trade": trade, "balance": round(portfolio['balance'], 2),
                "pnl_usd": round(pnl_usd, 2), "outcome": trade['outcome']}
    except HTTPException: raise
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
...
def get_trade_history(user_id: str):
    try:
        portfolio = get_or_create_portfolio(user_id); closed = portfolio['closed_trades']
        wins = [t for t in closed if t.get('outcome') == 'WIN']; total_pnl = sum(t.get('pnl_usd', 0) for t in closed)
        return {"user_id": user_id, "total_trades": len(closed), "wins": len(wins), "losses": len(closed) - len(wins),
                "win_rate": round(len(wins) / len(closed) * 100, 1) if closed else 0,
                "total_pnl": round(total_pnl, 2), "trades": closed}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        if RAILWAY_DB_URL:
            try:
                ...
                cur.execute("SELECT user_id, balance FROM paper_portfolios_v2 ORDER BY balance DESC LIMIT 10")
                rows = cur.fetchall(); conn.close()
                board = [{"user_id": uid, "balance": round(bal, 2), "return_pct": round((bal - STARTING_BALANCE) / STARTING_BALANCE * 100, 2), "trades": 0} for uid, bal in rows]
                return {"leaderboard": board}
            except: pass
        portfolios = load_paper_portfolios(); board = []
        for user_id, portfolio in portfolios.items():
            closed = portfolio['closed_trades']; wins = [t for t in closed if t.get('outcome') == 'WIN']
            ...
            board.append({"user_id": user_id, "return_pct": round(ret_pct, 2),
                         "win_rate": round(len(wins) / len(closed) * 100, 1) if closed else 0, "trades": len(closed)})
...
def reset_portfolio(user_id: str):
    try:
        new_portfolio = {'user_id': user_id, 'balance': STARTING_BALANCE, 'starting_balance': STARTING_BALANCE,
                        'open_trades': [], 'closed_trades': [], 'created_at': datetime.now().isoformat()}
        persist_portfolio(user_id, new_portfolio)
...
def warmup_cache():
    ...
    for symbol in ['BTC', 'ETH', 'AAPL', 'NVDA', 'TSLA', 'GLD']:
        try: get_live_price_only(symbol); print(f"  {symbol}: cached")
        except: pass
    print("Cache warm.")
    print("Running background backtests...")
    for symbol in ['BTC', 'ETH', 'AAPL', 'NVDA', 'TSLA', 'GLD']:
        try: run_backtest(symbol)
        except: pass
...
def get_agent_state():
    try:
        state = {'timestamp': datetime.now().isoformat(), 'assets': {}, 'macro': {}, 'news': [], 'fear_greed': 50}
        for symbol in ['BTC', 'ETH', 'AAPL', 'NVDA', 'TSLA', 'GLD']:
            try:
                price_data = get_live_price_only(symbol)
                if price_data:
                    regime = detect_market_regime(symbol); bt = _backtest_cache.get(symbol, {})
                    state['assets'][symbol] = {
                        'price': price_data['price'], 'change_24h': price_data['change_24h'], 'rsi': price_data['rsi'],
                        ...
                        'sentiment': price_data['sentiment'], 'fear_greed': price_data['fear_greed'], 'regime': regime,
                        'win_rate': bt.get('win_rate', 52.0), 'sharpe': bt.get('sharpe_ratio', 0.8), 'max_dd': bt.get('max_drawdown', -15.0)}
                    state['fear_greed'] = price_data['fear_greed']
            except: pass
        try:
            macro = get_macro_indicators(); summary = macro.get('summary', {})
            state['macro'] = {'crisis_score': summary.get('crisis_score', 0), 'overall': summary.get('overall', 'NORMAL'),
                             'burry_signals': summary.get('burry_signals', 0),
                             'yield_curve': macro.get('yield_curve', {}).get('signal', 'NORMAL'),
                             'vix': macro.get('vix', {}).get('current', 20), 'vix_signal': macro.get('vix', {}).get('signal', 'NORMAL'),
        ...
        try:
            state['news'] = [{'title': h['title'], 'tag': h['tag'], 'priority': h['priority']} for h in news_cache.get('headlines', [])[:20]]
        except: pass
...
def load_agent_reports():
    if os.path.exists(AGENT_REPORTS_FILE):
        try:
            with open(AGENT_REPORTS_FILE, 'r') as f: return json.load(f)
        except: return []
    return []

def save_agent_reports(reports):
    try:
        with open(AGENT_REPORTS_FILE, 'w') as f: json.dump(reports[:500], f, indent=2)
    except: pass
...
def receive_agent_report(report: AgentReport):
    ...
    if len(_agent_reports) > 500: _agent_reports.pop()
...
def update_positions(data: PositionsUpdate):
    global _live_positions
    _live_positions = {p['symbol']: p for p in data.positions}
    return {"received": True, "count": len(_live_positions)}
...
def stop_agents():
    global _agents_stopped; _agents_stopped = True
    return {"stopped": True, "message": "All agents halted"}

@app.post("/agents/resume")
def resume_agents():
    global _agents_stopped; _agents_stopped = False
    ...
    if _live_positions:
        ...
        try:
            ...
            for rep in _agent_reports:
                if rep.get('action') in ['BUY', 'SELL', 'OPEN'] and rep.get('symbol'):
                    sym = rep['symbol']
                    if sym not in seen:
                        seen.add(sym)
                        positions.append({'symbol': sym,
                                         ...
                                         'value_usd': 100, 'pnl_usd': 0})
        except Exception as e:
            print(f"Positions error: {e}")
    return clean_floats({"stopped": _agents_stopped, "timestamp": datetime.now().isoformat(), "positions": positions})
...
def get_live_portfolio():
    try:
        ...
        prices = {s: get_live_price_only(s) for s in symbols}
        open_trades = []
        unrealised_pnl = 0
        for p in open_pos:
        ...
        return {"balance": round(portfolio_value, 2), "starting_balance": 10000, "total_return_pct": total_return, "unrealised_pnl": round(unrealised_pnl, 2), "open_trades": open_trades, "open_count": len(open_trades), "closed_count": total_closed, "win_rate": win_rate, "total_realised_pnl": round(float(total_pnl), 2), "timestamp": datetime.now().isoformat()}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))
